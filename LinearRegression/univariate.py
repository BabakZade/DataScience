...
class Model:
    def __init__(self, x_train: np.ndarray, y_train: np.ndarray, ifDetail: bool) -> None:
        """
        Initialize the Model class with training data and settings.

        Parameters:
        -----------
        x_train : np.ndarray
            The feature/input training data, typically a 1D array.
        y_train : np.ndarray
            The target/output training data, typically a 1D array.
        ifDetail : bool
    ...
    def settings(self) -> None:
        """
        Set up the default settings for gradient descent parameters such as learning rate and convergence gaps.
        """
    ...
    def yHat(self, x_new: np.ndarray) -> np.ndarray:
        ...
        """
        y_new = self.weight * x_new + self.bias
        return y_new

    def calculateY_predict(self) -> None:
        """
        Calculate predicted values (`self.y_predict`) for the training data (`self.x_train`)
        using the current model parameters (`self.weight`, `self.bias`).
        """
        self.y_predict = self.weight * self.x_train + self.bias
    
    def trainModel(self) -> None:
        """
        Train the model using gradient descent, then compute predictions and Mean Squared Error (MSE).
        If detailed mode is enabled, plots the training progress.
        """
    ...
    def plotCountorForTraining(self) -> None:
        ...
        during the training process. This visualizes how the model converges.
        """
